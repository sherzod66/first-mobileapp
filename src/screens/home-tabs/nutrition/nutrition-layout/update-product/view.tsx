			...
			>
				<View style={styles.modalContainer}>
					...
					<Text style={{ color: 'white', marginBottom: 30 }}>
						Для добавления под-категории, выберите основную категорию нажав на окно "Категория"
						внизу. Для добавления категории, введите названия категории,игнорируя окно "Категория"
					</Text>
					<SelectPrimary
						title='Категория'
						data={categories.map(e => ({ label: e.name.ru, value: e._id })) || []}
				...
				{updateCategoryValue && (
					<View style={styles.modalContainer}>
						...
						<InputPrimary
							...
							containerStyle={styles.inputCont}
							onChange={v => setUpdateCategoryValue(prev => ({ ...prev, ru: v }))}
						/>
						<Text style={[styles.textOne, { marginVertical: 10 }]}>Название(UZ)</Text>
						<InputPrimary
							disablePlaceholder
							...
							onChange={v => setUpdateCategoryValue(prev => ({ ...prev, uz: v }))}
						/>
						<Text style={[styles.textOne, { marginVertical: 10 }]}>Название(EN)</Text>
						<InputPrimary
							disablePlaceholder
							...
							onChange={v => setUpdateCategoryValue(prev => ({ ...prev, en: v }))}
						/>
						<ButtonSecondary
