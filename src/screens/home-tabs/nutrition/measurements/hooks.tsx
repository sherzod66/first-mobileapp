...
export type MeasurementsScreenRouteProp = RouteProp<NutritionStackParamList, NUTRITION.MEASUREMENTS>
...
export const MeasurementsHooks = (apprenticeId = '') => {
	...
	const onAddCol = async () => {
		if (user) {
			...
			try {
				let res = await ApiService.put<Response<User>>(`/users/add-measurement-key/${user._id}`, {
					key: modalValue
				})
	...
	const onAddRow = async () => {
		if (user) {
			...
			try {
				const res = await ApiService.put<Response<User>>(`/users/add-measurement-row/${user._id}`)
