...
const getIsFinish = (scheduleWorkout: ScheduleWorkout | null, workoutIndex: number, i: number) => {
	const repeatAndWeight: string[] = []
	if (scheduleWorkout) {
		scheduleWorkout.plan.workouts[workoutIndex].forEach((w, iii) => {
			const { repeat, weight } =
				scheduleWorkout.results[workoutIndex][scheduleWorkout.activeWeek + i][iii][
					scheduleWorkout.results[workoutIndex][scheduleWorkout.activeWeek + i][iii].length - 1
				]
			if (repeat && weight) {
	...
	return (
		<View style={styles.container}>
							...
							<View style={{ flexDirection: 'row' }}>
								<View style={styles.workoutStake}>
									<TouchableWithoutFeedback
										onPress={() => setShow(prev => ({ ...prev, [ii]: !show[ii] }))}
									>
										<View style={[styles.titleColumn, { marginTop: 10 }]}>
											<Text style={styles.title}>{`${t('training')} ${ii + 1}`}</Text>
											<Icon
										...
										<>
											{ww.map((w, i) => (
												<TouchableOpacity
													...
													style={[styles.column, styles.itemsStart, !i && { borderTopWidth: 0 }]}
													onPress={() => onPressExercise(w.exercise)}
												>
													<Text numberOfLines={1} style={styles.textEllipsis}>
														{w.exercise?.title[i18n.language as 'ru' | 'en' | 'uz']}
													</Text>
												</TouchableOpacity>
											))}
										</>
								...
								<ScrollView key={ii} horizontal={true} showsHorizontalScrollIndicator={false}>
									<View
										style={[
											styles.main,
											!show[ii] && {
									...
									>
										{new Array(4).fill(1).map((a, i) => (
											<TouchableWithoutFeedback
												onPress={() => !showData.isFinished && onPress(ii, showData.activeWeek + i)}
												key={`${ii}/${showData.activeWeek + i}/s/${showData.plan?._id}`}
											>
												<View style={styles.weekStake}>
													{!showData.isFinished && (
														<View style={styles.titleColumn}>
															<Text style={[styles.text, getIsFinish(data, ii, i)]}>{`${t(
																'week'
															)} ${showData.activeWeek + i + 1}`}</Text>
														</View>
													)}
													{show[ii] && (
														<>
															{showData.plan.workouts[ii].map((w, iii) => {
																...
																const { repeat, weight } =
																	showData.results[ii][showData.activeWeek + i][iii][
																		showData.results[ii][showData.activeWeek + i][iii].length - 1
																	]
																if (weight && repeat) {
																	str = `${weight}/${repeat}`
																}
																...
																return (
																	<View
																		style={[styles.column, !iii && { borderTopWidth: 0 }]}
																		key={`${ii}/${showData.activeWeek + i}/${iii}/${
																			w.exercise?._id
																		}}`}
																	>
																		<Text
																			style={[
																				styles.text2,
																				{
																					color: str === '-' ? COLORS.GREY11 : COLORS.WHITE
																				}
																			]}
																		>
																			{str}
          ...
          {!!data && (
			...
			<Modal show={showModal} loading={modalLoading} onHide={onHide} onPress={onFinish} />
		</View>
	)
}
