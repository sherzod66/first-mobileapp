							...
							<ScrollView key={ii} horizontal={true} showsHorizontalScrollIndicator={false}>
								...
								>
									<View style={styles.workoutStake}>
										<TouchableWithoutFeedback onPress={() => setShow({ [ii]: !show[ii] })}>
									...
									{new Array(4).fill(1).map((a, i) => (
										...
										>
											<View style={styles.weekStake}>
												<View style={styles.titleColumn}>
													<Text style={[styles.text, getIsFinish(data, ii, i)]}>{`${t('week')} ${
														data.activeWeek + i + 1
													}`}</Text>
												</View>
												{show[ii] && (
														...
														{data.plan.workouts[ii].map((w, iii) => {
															...
															const { repeat, weight } =
																data.results[ii][data.activeWeek + i][iii][
																	data.results[ii][data.activeWeek + i][iii].length - 1
																]
															if (weight && repeat) {
																str = `${weight}/${repeat}`
															}
															...
															return (
																<View
																	...
																	key={`${ii}/${data.activeWeek + i}/${iii}/${w.exercise?._id}}`}
																>
																	<Text
																		style={[
																			styles.text2,
