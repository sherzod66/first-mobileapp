...
const MeasurementBox = ({ myMeasurements, onSet, onSetDate, onAdd }: IProps) => {
	...
	return !!myMeasurements.length ? (
		<View style={styles.container}>
				...
				<ScrollView horizontal showsHorizontalScrollIndicator={false}>
					<View>
						{myMeasurements.map((a, ii) => (
							<View key={`row-${ii}`} style={[styles.row, !!ii && styles.borderTop]}>
								<View style={styles.center}>
									{ii === 0 ? <Text style={styles.title}>{t('date')}</Text> : null}
									<TouchableOpacity
									...
									>
										<Text style={styles.text}>{moment(new Date(a.date)).format('DD.MM.YYYY')}</Text>
									</TouchableOpacity>
								</View>
								{a.data.map((d, i) => (
									<View style={styles.center} key={`col-${ii}-${i}`}>
										{ii === 0 ? <Text style={styles.title}>{d.key}</Text> : null}
										<TouchableOpacity
											onPress={() => onSet(ii, i)}
											style={[
												styles.col,
			...
			<ButtonPrimary text='+' onPress={onAdd} style={styles.btn} textStyle={styles.btnText} />
		</View>
	) : (
		<EmptyComponent />
	)
